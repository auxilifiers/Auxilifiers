    ...
    >
      <div className="relative w-full h-full transition-transform duration-700"
        ...
        <div className="relative w-full flex flex-col p-7 overflow-hidden"
          ...
          <p className="mb-5" style={{ fontFamily: "var(--font-body)", fontSize: 16, lineHeight: 1.65, color: "var(--color-text-muted)" }}>
            {pillar.description}
          </p>
          {/* Viz — bigger */}
          ...
          <div className="flex flex-wrap gap-2">
            {pillar.tags.map((tag) => (
              <button key={tag.id} className="tag flex items-center gap-1.5 transition-all duration-200" data-service={tag.id} onClick={() => flipTo(tag.id)}
                style={{ fontFamily: "var(--font-mono)", fontSize: 13, padding: "7px 14px", borderRadius: "var(--radius-pill)", border: "1px solid var(--color-border-strong)", color: "var(--color-cyan)", background: "rgba(0, 245, 255, 0.06)", letterSpacing: "0.04em", cursor: "none" }}>
                {tag.label}<span style={{ fontSize: 10, opacity: 0.6 }}>↗</span>
              </button>
            ))}
          </div>
        </div>
